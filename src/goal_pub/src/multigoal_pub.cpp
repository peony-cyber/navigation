...
GoalPublisher::GoalPublisher() : Node("multigoal_pub_node")
{
    ...
    // 3. 创建发布器
    goal_publisher_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("/goal_pose",  rclcpp::QoS(1).reliable().durability_volatile());
...
void GoalPublisher::publishNextGoal()
{
    if (goal_id < goals.size())
    {
        ...
        pose_msg.header.stamp = rclcpp::Time(0);
